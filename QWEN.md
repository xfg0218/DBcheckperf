...
| 版本 | 日期 | 类型 | 主要内容 |
|------|------|------|----------|
| v1.7.0 | 2026-04-08 | 功能集成 | advisor/history/progress 集成、JSON 报告、基线对比 |
...
### v1.7.0 详细说明 (2026-04-08)

**核心改进：已开发模块集成到主流程**

v1.6.0 引入的三个核心模块（advisor/history/progress）代码已写好但**完全没有在 main.go 中集成和调用**。v1.7.0 修复了这个问题。
...
2. **历史数据自动保存** 📊
   - 测试完成后自动保存到 `~/.dbcheckperf/history/`
   - JSON 格式存储，包含磁盘/内存/网络/延迟摘要
   - 支持 `--no-save` 禁用自动保存

3. **基线自动对比** 📈
   - `--baseline` 选项自动加载上次记录对比
   - 显示磁盘/内存/网络/延迟的变化百分比
   - 彩色 emoji 标识性能升降（🟢提升/🟡下降/🔴严重下降）

4. **JSON 报告输出** 📄
   - `--report json` 输出结构化 JSON 报告
   - 包含系统信息、磁盘/内存/网络/延迟结果
   - 包含 advisor 评分和告警信息
   - 便于集成到监控系统和 CMDB

**新增命令行选项**:
- `--baseline`: 自动与上次测试结果对比
- `--no-save`: 禁用自动保存测试记录
- `--report json`: JSON 报告输出（之前有选项但无实现）
