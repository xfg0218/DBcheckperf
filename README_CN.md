...
- **HTML 报告生成**: 响应式设计，渐变紫色主题，包含报告概览、系统信息、测试结果等部分
- **历史数据对比**: JSON 格式存储测试记录，支持标签搜索，自动对比性能变化百分比
- **智能告警与优化建议**: 性能评分（0-100 分）、健康状态评估、自动检测性能瓶颈、提供优化建议
- **进度条和预估时间**: 实时进度条显示、已用时间和 ETA 预估、步骤跟踪
- **JSON 报告输出**: 结构化数据，便于集成到监控系统

### v1.5.0 新增功能

- **fio/iperf3 自动部署**: 支持 6 种包管理器（apt-get/yum/dnf/zypper/apk/pacman），自动检测和安装
- **测试覆盖率提升**: memory 模块 86%，utils 模块 56.4%，总体覆盖率~25%
- **CI/CD 集成**: GitHub Actions 自动化测试、多平台编译、Codecov 覆盖率集成
...
- **智能顾问与告警**: 自动性能评分（0-100分）、健康状态评估、三级告警（INFO/WARNING/CRITICAL）、具体优化建议
- **历史数据管理**: 自动保存测试结果、基线对比、趋势跟踪、标签搜索
- **JSON 报告输出**: 结构化 JSON 报告，便于集成到监控系统和 CMDB
- **基线对比**: `--baseline` 选项自动对比上次测试结果，显示性能变化百分比
...
| `--report <格式>` | 报告格式：html,json,table（默认 table） | table |
| `--output <路径>` | 报告输出文件路径 | 当前目录 |
| `--baseline` | 自动与上次测试结果对比（基线对比） | false |
| `--no-save` | 禁用自动保存测试结果到历史记录 | false |
| `--version` | 显示版本号 | - |
| `-?` | 显示帮助信息 | - |
